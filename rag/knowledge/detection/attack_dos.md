...
Slowloris. The model cannot reliably separate the two: they rest on nearly the same features, and the measured similarity and confusion rate are reported with the finding. Always report the pair. DDoS, which is the same behaviour from many sources.
